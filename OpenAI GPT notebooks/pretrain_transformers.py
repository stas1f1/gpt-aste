...
class BlockByBlockTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        assert os.path.isfile(file_path)
        ...
        with open(file_path, encoding="utf-8") as f:
            blocks = [block for block in f.read().split('\n\n') if (len(block) > 0 and not block .isspace())]
        
        
        
        self.examples = tokenizer.batch_encode_plus(blocks, add_special_tokens=True, max_length=block_size)["input_ids"]
...
class BlockByBlockWithNewlineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        assert os.path.isfile(file_path)
        # Here, we do not cache the features, operating under the assumption
        # that we will soon use fast multithreaded tokenizers from the
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.block_by_block_with_newline:
        return BlockByBlockWithNewlineTextDataset(tokenizer, args, file_path=file_path, block_size=args.block_size)
    elif args.block_by_block:
        return BlockByBlockTextDataset(tokenizer, args, file_path=file_path, block_size=args.block_size)
...
def main():
    ...
    parser.add_argument(
        "--block_by_block",
        action="store_true",
        help="Whether distinct blocks of text (split by empty line) in the dataset are to be handled as distinct sequences.",
    )
    parser.add_argument(
        "--block_by_block_with_newline",
        action="store_true",
        help="Whether distinct blocks of text (split by empty line) in the dataset are to be handled as distinct sequences"
             "and should contain newline in the end of an example",
    )
    parser.add_argument(
